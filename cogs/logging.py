...
class Logging(commands.Cog):
    ...
    async def on_member_remove(self, member):
        ...
        await self._log_stuff(member.guild, f":outbox_tray: **{member}** ({member.id}) hat den Server verlassen.")
    ...
    async def on_member_ban(self, _, member):
        ...
        await self._log_stuff(member.guild, f":no_entry_sign: **{member}** ({member.id}) wurde gebannt.")
    ...
    async def on_raw_message_delete(self, payload):
        ...
        if not payload.cached_message:
            await self._log_stuff(guild, f":recycle: Nachricht ({payload.message_id}) in Channel **{channel}** "
        ...
        if message.author.bot and str(logchannelid) == str(channel.id):
            ...
            await self._log_stuff(guild, message.clean_content, delete_after=delete_after)
        else:
            await self._log_stuff(guild, f":recycle: Nachricht ({message.id}) von "
